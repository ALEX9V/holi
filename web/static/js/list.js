...
export default function renderList(opts) {
    console.log(opts, "")
 const mess = opts.item
 const div = document.createElement('div')
 div.className='hjkhkj'
   ...
   var z=0
       ...
       for(var j= 0; j< opts.items.length; j++){
           z=0
            for(var i= 0; i< mess.length; i++){
                if (mess[i].Actors[0]==opts.items[j].UsersnameR){
                    z += 1
                    opts.items[j].count=z
                    opts.items.push()
    ...
    if (opts.items.length === opts.pageSize) {
        ...
        setTimeout(() => {
            ...
            feed.el.insertAdjacentElement(opts.reverse ? "beforeend" : "afterbegin", loadMoreButton)
