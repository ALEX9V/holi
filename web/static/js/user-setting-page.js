import { doGet, doPostAvatar,doPost } from "/js/http.js"
const template = document.createElement('template')
const b =localStorage.getItem('avatar_image')
const c = b.replace(/"([^"]+(?="))"/g, '$1')
template.innerHTML = `

<div class="container">
<h1>Настройки</h1>
<h2>Изменить персональные данные учетной записи</h2>
</br>
<h2>Баннер профиля</h2>
<form name="fileinfo"  id="post-form"  >
<img style ="border-radius: 50%;"src="${c}"/>
<input type="file" name="upfile"  id="upfile" style="box-shadow:none;">
<button id="post-form-button">Publish</button>
</form>
</br>
<form name="fileinfo_sh"  id="post-form_sh"  >
<input type="text"  id="fnamer" placeholder="Имя"><input type="text" style="    margin-left: 40px;"  id="lnamer" placeholder="Фамилия">
</br>
</br>
<input type="text"  id="opsh" placeholder="Опыт работы">
</br>
</br>
<input type="text"  id="scils" placeholder="Навыки">
</br>
<input type="text"  id="age" placeholder="Возраст">
</br>
<input type="text" id="city" placeholder="Страна">
</br>
<input type="password"  id="oldpass" placeholder="Старый пароль">
<input type="password" id="newpass" placeholder="Новый пароль">
<h2>Пол</h2>
<input type="radio" id="btpp" name="age" value="0">
<input type="radio" id="btpp" name="age" value="1" checked="checked">
<input type="radio" id="btpp" name="age" value="2">
</br>
<textarea id="opis" placeholder="О себе ....."></textarea>
</br>
<button id="post-form-button_sh">Publish</button>
</form>
</div>
...
export default async function renderNotFoundPage() {
    ...
    gp.innerHTML =`<link rel="stylesheet" type="text/css" href="/css/RLsu_p6978676549876789.css">`
    chat.innerHTML=`<img id='im-gg-ll' src='/js/svg/usersetting.svg'/>
    <div class="bd-gg-ll">
    <h2> ◆ Здесь вы можите Внести изменения в профиль.</h2>
    <span> Вы можите изменить данные полностью или по отдельности.</span>
    </div>`
    ...
    if (d.mm_34_pp_s243 == true){
       ...
       gp.innerHTML =`<svg height="200px" id="svg_pp_mm" viewBox="0 0 512.14969 512" fill=" #8198e9" width="200px" xmlns="http://www.w3.org/2000/svg"
       style="
   ...
   "
       ...
       </br><h1 style="margin: 0 0 0 20%;"> Упс, Вы достигли лимита =(</h1><p style="margin: 0 0 0 15%; color:silver;">(Внимание: вы можите изменять данные не более 1  раза в день) </p>
       `
    ...
    const postForm_sh =/** @type {HTMLFormElement} */ (page.getElementById('post-form_sh'))
    const fnamer = page.getElementById('fnamer')
    const lnamer = page.getElementById('lnamer')
    const titopshle = page.getElementById('opsh')
    const scils = page.getElementById('scils')
    const ages = page.getElementById('age')
    const city = page.getElementById('city')
    const opis = page.getElementById('opis')
    const oldpass = page.getElementById('oldpass')
    const newpass = page.getElementById('newpass')
    const age = page.querySelector('input[name="age"]:checked')
 ...
 const onpostubmitsh = async ev => {
    ev.preventDefault() 
const ln = lnamer.value
const fn = fnamer.value
const opi =titopshle.value
const sci=scils.value
const op= opis.value
const pol=  age.value
const ager=  ages.value
const citys=  city.value
const olp=  oldpass.value
const nwp=  newpass.value
try {
    console.log(postForm_sh)
await publishPostSH({ opi, sci, op, pol, ager, citys, ln, fn, olp, nwp})
 ...
 postForm_sh.addEventListener('submit', onpostubmitsh)
...
async function publishPost(input) {
       await doPostAvatar("/api/users/update", input) 
}

async function publishPostSH(input) {
    await doPost("/api/users/updates", input)
