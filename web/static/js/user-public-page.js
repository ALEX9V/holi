...
<h3>Уровень Опыта</h3>
<p><input id="radio" type="radio" name="age" value="0">Новичок</p>
<p><input id="radio" type="radio" name="age" value="1" >Среднее</p>
<p><input id="radio" type="radio" name="age" value="2">Эксперт</p>
<h3>Доступ</h3>
<p><input id="radio" type="radio" name="access" value="0">Временная работа</p>
<p><input id="radio" type="radio" name="access" value="1">На длительный срок</p>
</br>
<h3>Материалы для взрослых</h3>
<p><input id="radio" type="radio" name="worktp" value="0">да</p>
<p><input  id="radio" type="radio" name="worktp" value="1">нет</p>
</br>
<h3>Часы в неделю</h3>
<p><input id="radio" type="radio" name="hourtp" value="0"> менее 30 часов</p>
<p><input id="radio" type="radio" name="hourtp" value="1">более 0 часов</p>
</br>
...
export default async function rendeUploadPage() {
    const d =  await mime()
    ...
    chat.innerHTML+=`<img id='im-gg-ll' src='/js/svg/publish.svg'/>
    <div class="bd-gg-ll">
    <h2> ◆ Здесь вы опубликовать вакансии.</h2>
    <span> Добавляйти теги для того, чтобы соискателям было легче вас найти.(Для того, чтобы открыть стартап добавте тег startapp ).</span>
    </div>`
    ...
    try {
        ...
        const timelineItem = await publishPost( {content, ages, acess1, titles, tags, amoti, access, worktp, hourtp } )
        const contents = timelineItem.Post.lk_j21_dv_i223
        await publishImage( postForm, contents)
 ...
 */
async function publishPost( input2, input3, input4, input5) {
    console.log("input2", input2, input3, input4, input5)
    const timelineItem =  await doPost("/api/post", input2)
    return timelineItem
}
      
   
      async function publishImage( input, inputs) {
        input.content.value = inputs
        await doPostAvatar("/api/users/upload", input)
}
function mime( ) {
